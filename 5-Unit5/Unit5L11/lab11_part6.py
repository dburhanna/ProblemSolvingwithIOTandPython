...
# set location metadata value: "lat, lon, elev"
location_data = "41.82254,-80.98346, 626.6"
...
while True:
    ...
    if (time.monotonic() - last) >= 10:
            ...
            temp_value = str(bmp280.temperature*1.8 + 32)
            ...
            humid_value = str(sht31d.relative_humidity)
            print("Publishing {0} to feed: {1}".format(humid_value,humidity_feed))
            io.publish(humidity_feed,humid_value,location_data)

            #pres_value = randint(950,1150)
            pres_value = str(bmp280.pressure)
            print("Publishing {0} to feed: {1}".format(pres_value,pressure_feed))
            io.publish(pressure_feed,pres_value,location_data)
